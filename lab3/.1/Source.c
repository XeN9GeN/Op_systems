...
int copy_file_reversed(const char* src_path, const char* dest_path) {
    ...
    while (remaining_read_bytes > 0 && buffer != NULL) {
        ...
        fseek(source_filename, remaining_read_bytes - to_read, SEEK_SET);
        if (fseek == ERROR_VAL) {
        ...
        fwrite(buffer, 1, n_read, dest_filename);
...
void prepare_and_process(int argc, char* argv[]) {
    ...
    if (mkdir(dest_dir, DIR_MODE) == ERROR_VAL) {
        perror("ОШИБКА СОЗДАНИИ ДИРЕКТОРИИ");
        return FAILURE;
    }
